...
def _ilp_rotation(attending, quarterly_gks, player_ranks, split_pairs, synergy_pairs, formation_key, seed, subs_per_quarter):
    ...
    # Lower bound: activate (force to 1) when any two distinct types are both played.
    # Upper bound: prevent activation unless at least 2 types are played — without this
    # the solver can claim the 4-pt bonus for free even when only 1 type is played.
    for p in attending:
